...
@section('breadcrumb')
    ...
    <div class="breadcrumb-item">
        <i class="fas fa-chevron-right"></i>
        <span>{{ __('Edit') }}: {{ $platform->name }}</span>
    </div>
@endsection
...
<div class="form-container fade-in">
    <form action="{{ route('admin.platforms.update', $platform) }}" method="POST" id="platformForm">
        @csrf
        @method('PATCH')
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" 
                                  ...
                                  rows="4" 
                                  placeholder="{{ __('Brief description about the platform (optional)') }}">{{ old('description', $platform->description) }}</textarea>
                        @error('description')
                            <div class="form-error">
                                <i class="fas fa-exclamation-circle"></i>
                                {{ $message }}
                ...
                <div>
                    <div class="preview-card">
                        ...
                        <div style="color: var(--admin-secondary-600); font-size: 0.875rem;" id="previewPercentage">
                            +{{ $platform->price_percentage }}% {{ __('price increase') }}
                        </div>
                        
                        <div class="platform-stats">
                            <div class="stat-item">
                                <div class="stat-number">{{ $platform->dossiers()->count() }}</div>
                                <div class="stat-label">{{ __('Dossiers') }}</div>
                            </div>
                            
                            <div class="stat-item">
                                <div class="stat-number">{{ $platform->orderItems()->count() }}</div>
            ...
            <div class="form-group">
                ...
                <div class="percentage-input-group">
                    <input type="number" 
                           ...
                           value="{{ old('price_percentage', $platform->price_percentage) }}" 
                           min="0" 
                           max="100" 
                           step="0.01"
                           placeholder="0"
                ...
                <div class="price-calculator" id="priceCalculator">
                    ...
                    <div class="calc-row">
                        <span class="calc-label">{{ __('Platform Fee') }} (<span id="calcPercentage">{{ $platform->price_percentage }}</span>%):</span>
                        <span class="calc-value">+<span id="calcFee">{{ number_format(10 * ($platform->price_percentage / 100), 2) }}</span> JD</span>
                    </div>
                    
                    <div class="calc-row">
                        <span class="calc-label">{{ __('Final Price') }}:</span>
                        <span class="calc-value"><span id="calcFinal">{{ number_format(10 + (10 * ($platform->price_percentage / 100)), 2) }}</span> JD</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="number" 
                           ...
                           value="{{ old('order', $platform->order) }}" 
                           min="0" 
                           placeholder="0">
                    @error('order')
                        <div class="form-error">
                ...
                <div class="form-group">
                    ...
                    <div class="status-toggle">
                        ...
                        <div class="toggle-switch {{ $platform->is_active ? 'active' : '' }}" onclick="toggleStatus()" id="statusToggle">
                            <input type="hidden" name="is_active" value="{{ $platform->is_active ? '1' : '0' }}" id="statusInput">
                        </div>
...
function updatePreview() {
    ...
    const percentage = document.getElementById('price_percentage').value || '{{ $platform->price_percentage }}';
...
document.getElementById('platformForm').addEventListener('submit', function(e) {
    ...
    setTimeout(() => {
        ...
        submitBtn.innerHTML = '<i class="fas fa-save"></i> {{ __('Update Platform') }}';
    }, 5000);
});
